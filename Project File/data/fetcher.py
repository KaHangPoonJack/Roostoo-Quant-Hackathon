        ...
        try:
            ...
            if not candles:
                if attempt < max_retries - 1:
                    print(f"⚠️  No data received (attempt {attempt+1}/{max_retries})")
                    print(f"   Retrying in 3 seconds...")
                    time.sleep(3)
                    continue
                else:
                    raise Exception("No data returned from Binance after 3 attempts")
            ...
            if attempt < max_retries - 1:
                ...
                print(f"❌ Failed to load Binance data after {max_retries} attempts")
                print(f"\n💡 TROUBLESHOOTING:")
                print(f"   1. Check your internet connection")
                print(f"   2. Try: ping api.binance.com")
                print(f"   3. Check if Binance is blocked by firewall/antivirus")
                print(f"   4. Try using a VPN if Binance is restricted in your region")
                print(f"   5. Wait a moment and restart the bot\n")
                raise
