...
# ================= ORDER RATE LIMITER =================
# Simple rate limiter ONLY for place_order to prevent 429 errors
# All other API calls (balance, query, cancel) are NOT limited

class _OrderRateLimiter:
    """
    Simple rate limiter for place_order only.
    Ensures orders are placed with at least 1 second between them.
    """
    ...
    def wait_before_order(self):
        """Wait if necessary before placing an order"""
        with self.lock:
            now = time.time()
...
# Global order rate limiter instance
...
def place_order(pair_or_coin: str, side: str, quantity: str,
    ...
    """
    Place a LIMIT or MARKET order with rate limiting (1 second between orders)
    Rate limiting applies ONLY to this function, not to balance/query/cancel
    """
    # Wait to maintain minimum interval between orders
    _order_rate_limiter.wait_before_order()
